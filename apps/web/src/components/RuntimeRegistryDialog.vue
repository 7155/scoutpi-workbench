...
import { Activity, Blocks, BookOpen, Braces, Clock3, Coins, Database, Gauge, HardDriveDownload, LoaderCircle, Power, Radar, Radio, Rocket, Save, ServerCog, ShieldCheck, Upload, X } from "lucide-vue-next";
import { api } from "../api";
import type { AgentRunSummary, EarthBackendManifest, EarthBackendProbe, EarthSkillSummary, RegisteredAdapter, RuntimeApproval, RuntimeTelemetrySummary } from "../types";

const props = defineProps<{ open: boolean; saving: boolean; adapters: RegisteredAdapter[]; skills: EarthSkillSummary[]; backendManifests: EarthBackendManifest[]; backendProbes: Record<string, EarthBackendProbe>; telemetry?: RuntimeTelemetrySummary; agentRuns: AgentRunSummary[]; approvals: RuntimeApproval[] }>();
