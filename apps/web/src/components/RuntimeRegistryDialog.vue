    ...
    <section class="registry-dialog" role="dialog" aria-modal="true" aria-label="Runtime registry">
      ...
      <div v-else-if="active === 'context'" class="context-view">
        <section class="context-column">
          <div class="telemetry-heading"><strong>Context packs</strong><span>{{ contextPacks.length }}</span></div>
          <article v-for="pack in contextPacks.slice(0, 10)" :key="pack.packId" class="context-pack-row">
            <div class="context-row-head"><BrainCircuit :size="15" /><code>{{ pack.packId }}</code><time>{{ formatTime(pack.createdAt) }}</time></div>
            <div class="context-budget"><span :style="{ width: `${Math.min(100, pack.budget.maxTokens ? pack.budget.deliveredTokens / pack.budget.maxTokens * 100 : 0)}%` }"></span></div>
            <div class="context-row-meta"><span>{{ pack.budget.selectedCount }}/{{ pack.budget.candidateCount }} items</span><span>{{ pack.budget.deliveredTokens }}/{{ pack.budget.maxTokens }} tokens</span><span v-if="pack.budget.truncated">truncated</span></div>
            <p v-if="pack.items[0]">{{ pack.items[0].text }}</p>
            <small>{{ pack.sourceProviders.join(' · ') || (pack.detectedMemoryTools.length ? `delegated: ${pack.detectedMemoryTools.join(' · ')}` : 'no provider data') }}</small>
          </article>
          <div v-if="!contextPacks.length" class="empty"><BrainCircuit :size="25" /><span>No context packs yet.</span></div>
        </section>
        <section class="context-column context-runtime-column">
          <div class="telemetry-heading"><strong>Durable checkpoints</strong><span>{{ checkpoints.length }}</span></div>
          <article v-for="checkpoint in checkpoints.slice(0, 6)" :key="checkpoint.checkpointId" class="context-state-row">
            <History :size="14" /><div><code>{{ checkpoint.sessionId }}</code><small>{{ checkpoint.state }} · r{{ checkpoint.revision }} · {{ checkpoint.references.length }} refs</small></div><span :class="['context-state', { recoverable: checkpoint.recovery.recoverable }]">{{ checkpoint.recovery.recoverable ? 'recover' : 'settled' }}</span>
          </article>
          <div v-if="!checkpoints.length" class="empty compact"><History :size="22" /><span>No durable checkpoints yet.</span></div>
          <div class="telemetry-heading context-writeback-heading"><strong>Writeback review</strong><span>{{ contextWritebacks.length }}</span></div>
          <article v-for="writeback in contextWritebacks.slice(0, 8)" :key="writeback.writebackId" class="context-state-row">
            <CircleCheck v-if="writeback.state === 'approved'" :size="14" />
            <CircleX v-else-if="writeback.state === 'rejected'" :size="14" />
            <Hourglass v-else :size="14" />
            <div><code>{{ writeback.writebackId }}</code><small>{{ writeback.candidates.length }} candidates · {{ writeback.providerTargets.join(' · ') || 'provider outbox' }}</small></div><span :class="['writeback-state', writeback.state]">{{ writeback.state }}</span>
          </article>
          <div v-if="!contextWritebacks.length" class="empty compact"><Hourglass :size="22" /><span>No writeback candidates yet.</span></div>
        </section>
      </div>
...
import { Activity, Blocks, BookOpen, Braces, BrainCircuit, CircleCheck, CircleX, Clock3, Coins, Database, Gauge, HardDriveDownload, History, Hourglass, LoaderCircle, Power, Radar, Radio, Rocket, Save, ServerCog, ShieldCheck, Upload, X } from "lucide-vue-next";
import { api } from "../api";
...
const tabs = computed(() => [
  ...
  { id: "context", label: "Context", icon: markRaw(BrainCircuit), count: props.contextPacks.length + props.contextWritebacks.length },
  { id: "telemetry", label: "Telemetry", icon: markRaw(Activity), count: props.agentRuns.length },
]);
...
nav { display: grid; grid-template-columns: repeat(5, 1fr); border-bottom: 1px solid #dfe4e1; }nav button { display: flex; align-items: center; justify-content: center; gap: 6px; border: 0; border-right: 1px solid #e2e6e3; border-radius: 0; padding: 9px; background: #f7f9f7; color: #68756e; }nav button:last-child { border-right: 0; }nav button.active { box-shadow: inset 0 -2px #1f6846; background: #fff; color: #1f6846; font-weight: 700; }nav span { border-radius: 9px; padding: 1px 5px; background: #e7ece9; font-size: 9px; }
...
.context-view { display: grid; grid-template-columns: 1.12fr .88fr; min-height: 0; overflow: hidden; }.context-column { min-width: 0; overflow: auto; padding: 16px 18px 20px; }.context-runtime-column { border-left: 1px solid #e0e5e2; background: #fbfcfb; }.context-pack-row { display: grid; gap: 7px; border-bottom: 1px solid #e1e6e2; padding: 11px 0; }.context-row-head { display: grid; grid-template-columns: 18px minmax(0, 1fr) auto; gap: 7px; align-items: center; }.context-row-head svg { color: #2f6fad; }.context-row-head code, .context-state-row code { overflow: hidden; color: #315b48; font-size: 9px; text-overflow: ellipsis; white-space: nowrap; }.context-row-head time { color: #7d8982; font-size: 8px; }.context-budget { height: 3px; overflow: hidden; background: #e5ebe7; }.context-budget span { display: block; height: 100%; background: #2f7d59; }.context-row-meta { display: flex; flex-wrap: wrap; gap: 9px; color: #718078; font-size: 8px; text-transform: uppercase; }.context-pack-row p { display: -webkit-box; overflow: hidden; margin: 0; color: #445249; font-size: 9px; line-height: 1.45; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }.context-pack-row > small { overflow: hidden; color: #819087; font-size: 8px; text-overflow: ellipsis; white-space: nowrap; }.context-state-row { display: grid; grid-template-columns: 18px minmax(0, 1fr) auto; gap: 8px; align-items: center; border-top: 1px solid #e2e7e4; padding: 9px 0; }.context-state-row > svg { color: #527363; }.context-state-row > div { display: grid; min-width: 0; gap: 2px; }.context-state-row small { overflow: hidden; color: #76837b; font-size: 8px; text-overflow: ellipsis; white-space: nowrap; }.context-state, .writeback-state { border-radius: 8px; padding: 2px 6px; background: #e9eeeb; color: #66746c; font-size: 8px; text-transform: uppercase; }.context-state.recoverable, .writeback-state.pending { background: #fff0cf; color: #8a6111; }.writeback-state.approved { background: #dff1e7; color: #216845; }.writeback-state.rejected { background: #f8e2e3; color: #943f48; }.context-writeback-heading { margin-top: 18px; }
...
@media (max-width: 720px) { .registry-content { grid-template-columns: 1fr; overflow: auto; }.registry-list { overflow: visible; }.json-editor { border-top: 1px solid #e0e5e2; border-left: 0; }.backend-grid { grid-template-columns: 1fr; }.context-view { grid-template-columns: 1fr; overflow: auto; }.context-column { overflow: visible; }.context-runtime-column { border-top: 1px solid #e0e5e2; border-left: 0; }.telemetry-metrics { grid-template-columns: repeat(2, 1fr); }.telemetry-columns { grid-template-columns: 1fr; }.agent-ledger { border-top: 1px solid #e0e5e2; border-left: 0; }nav button { font-size: 0; }nav button span { font-size: 9px; } }
</style>
