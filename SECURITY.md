...
- The Wisdom Weasel Context Provider is opt-in through operator-owned absolute paths. It executes one reviewed Python entrypoint, allows query only, caps request/output/items/time, disables raw event candidates, rejects secret-looking text, and never enables the IME debug server's raw-text mode or writes to its database.
