...
- 设计 Pi 原生 Agent 运行时，将数据注册、规划、计算、导出、证据和工作流能力收敛为 3 个动态网关工具；相较一次性披露 7 个 runtime contract 的固定基线，tool schema 估算从 1,341 降至 380 tokens，减少 71.66%。
...
- Consolidated registry, planning, compute, export, evidence and workflow capabilities into three dynamically activated Pi gateway tools; reduced the fixed-fixture schema estimate from 1,341 to 380 tokens (71.66%) against eagerly disclosing seven runtime contracts.
