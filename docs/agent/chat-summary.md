...
### 2026-07-11 - Interview-first evaluation milestone

- Added a content-minimal Evaluation Store/API/UI that displays Pi RPC, benchmark, end-to-end and recovery evidence without adding a fourth Pi tool.
- Added deterministic harnesses for three-gateway schema disclosure, Context Pack budgeting, workflow replay, generic Browser Evidence-to-EarthStory flow and kill/restart recovery.
- Fixed-fixture reductions are 71.66% tool schema, 72.45% context delivery and 66.67% external workflow calls; the values are generated by the harness and must be re-run after changes.
